...
# Generate C header file. Morse encodings are generated in array form, where the index
# is the deicmal ASCII value of character which the encoding refers to.
print $head "/* Matthew Blanchard\n";
print $head " * ECE 331\n";
print $head " * 2/18/2017\n";
print $head " * Title: encoding.h\n";
print $head " * Description: Morse encodings of all 256 ASCII characters. Each bit represents\n";
print $head " *         one time unit, where '1' is high and '0' is low\n";
print $head " * Generated by encoding.pl\n";
print $head " */\n\n";
...
print $head "unsigned int morse[256] = {\n";

# Print each encoding on its own line for readability.
for (my $i = 0; $i < 255; $i++) {
        printf $head ("%-40s", "        $encode{$i},");
        print  $head "// $i\n";
}

printf $head ("%-40s", "        $encode{255}};");
print  $head "// 255\n";
